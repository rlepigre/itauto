...
\supplement{https://gitlab.inria.fr/fbesson/itauto}
...
be slower than \icoq{tauto}.) These decision procedures are usually
...
Lemma Ex2: forall (A:Type) (a b c:A), a = b -> b = c -> (a = c -> p) -> p.
  \end{minted}
  For \icoq{Ex1} and \icoq{Ex2}, \icoq{intuition lia} and \icoq{intuition congruence} fail whereas
  the goal can be decided using a combination of
  propositional logic with the theory of linear arithmetics and
  the theory of equality, respectively.  However, the \icoq{intuition} part does
  nothing and therefore calls the leaf tactics on the unmodified goal.  For \icoq{Ex1},
  \icoq{lia} abstracts away non-arithmetic propositions\footnote{A propositional variable is replaced by \icoq{True} or \icoq{False} depending on its polarity.} and is left
...
\begin{example}[Spurious Leaf Tactic Calls]
...
Lemma Ex3: forall (A:Type) (x y:A) (p q:Prop),
  ...
  \end{minted}
  In this case, \icoq{intuition congruence} performs a case analysing
...
\icoq{LAT i} represents the propositional variable \icoq{i}.
%
\icoq{LOP o} $[f_1; \dots; f_n]$ where \icoq{o} $\in$\{\icoq{AND},
\icoq{OR}\} represents a n-ary conjunction and disjunction, respectively.
%
...
treatment. %In particular, the set of flat clauses
...
represent a propositional variable by a hash-consed formula (\icoq{HFormula} = \icoq{HCons.t LForm}). As a
...
is decidable. Therefore, \icoq{intro_impl: HFormula -> (list literal *
  HFormula)} takes as input a hash-consed formula and returns a pair
...
\icoq{is_dec} predicate is actually implemented by a boolean flag
...
is \icoq{False}, we also generate the full clausal form
\[
...
\[
  c = \{| \icoq{watch1} := \neg f ; \icoq{watch2} := \neg x ; \icoq{unwatched} := [\neg y_1;\neg y_2;\dots;\neg y_n]|\}
\]
The \icoq{reduce} function aims at finding in the list
$[\neg y_1;\neg y_n;\dots;\neg y_n]$ the first literal which negation is not assigned in the
current set of assigned literals \icoq{s} and return as a clause the
rest of the
...
%
The \icoq{case_split} algorithm takes as input a clause \icoq{cl} and
returns a \icoq{ProverT}.  It performs a case-analysis over all the
...
require \icoq{f} and all the literals are assigned in the current
...
\icoq{d} but instead the dependencies of the clause \icoq{cl} is added
just after the \icoq{case_split} call.
...
(\icoq{LIMPL a b}).
%
Remember that for such literals, the CNF rule \icoq{IMPL+} is incomplete if all the \icoq{a}s are not \emph{classical} propositions.
%
...
\item the updated hash-cons map \icoq{hm'} contains more has-consed formulae than \icoq{hm}.
\end{itemize}
Typically, the clause \icoq{cl'} is a conflict clause and therefore
the literals of \icoq{cl'} are including in those of
\icoq{cl}. However, the theory solver may also perform theory
...
For most of our proof state transformation, say \icoq{T}, the correctness lemma has the form
\[
...
%
Remember that the theory prover \icoq{thy_prover} (see Section~\ref{sec:thy-reasoning})
...
The first sub-case is solved by theory reasoning (\icoq{x=0 -> x >= 0}
...
For Ex~\ref{ex:too-many-calls}, the goal is solved by \icoq{intuition
  congruence} using two identical calls to the leaf tactic \icoq{congruence}.
%
Because our SAT solver thread learnt clauses along the computation,
the same goal is solved by \icoq{itauto congruence} by calling \icoq{congruence} only once.
...
%
The tactics \icoq{tauto} is the least scalable and reaches the timeout quickly.
%
...
\icoq{tac} $\in$ \{\icoq{congruence}, \icoq{lia}, \icoq{idtac}, \icoq{auto}, \icoq{eauto} \}.
%
...
majority  of the  goals. For  a few  of them,  \icoq{itauto} fails.  A
...
\begin{example} The following goal is solved by \icoq{intuition congruence}.
  ...
  recursively so that an hypothesis \icoq{x:Z} is explicitly
...
\icoq{itauto} as the leaf tactic.
...
\icoq{itauto} outperforms the historic tactics for only 13\% of the
...
goals, the running time also differ for at most 1ms. \icoq{itauto}
...
%
For instance, the maximum running time of \icoq{tauto} takes 7.26s
...
\icoq{itauto} needs to be optimised and how the time is spent between
