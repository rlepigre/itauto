\documentclass[utf8,a4paper,UKenglish,cleveref, autoref, thm-restate]{lipics-v2021}
\usepackage[]{minted}
\usepackage{stmaryrd}
...
\setminted[coq]{escapeinside=\#\#,mathescape=true}
...
equality with constructors. % that is useful to reason about inductive types.
Another tactic that is worth mentioning is \icoq{intuition tac}\footnote{\url{https://coq.inria.fr/refman/proof-engine/tactics.html\#coq:tacn.intuition}};
...
\end{minted}
  ...
  \icoq{x=0#$\to$#(x>=0#$\to$#True)#$\to$#False}.
  For \icoq{Ex2}, \icoq{congruence} abstracts away
  propositions and is also left with a non-theorem:
  \icoq{a=b#$\to$#b=c#$\to$#False}.
\end{example}
Besides completeness, \icoq{intuition tac} may also call the leaf tactic \icoq{tac}
...
solver. % thus preventing redundant theory calls.
% 
...
%solver as a novel propositional clause.
...
%where we compare with the existing Coq tactics.
%
...
\icoq{LAT} $i$ represents the propositional variable \icoq{p}$_i$.
%
...
%
The negation of a formula $f$ is encoded by \icoq{LIMPL} $[f]$ \icoq{LFF}.
%
...
to an environment \icoq{e:int #$\to$# Prop} mapping indexes \emph{i.e.}, propositional variables, to
propositions.
\[
  \begin{array}{lcl}
...
is decidable. Therefore, \icoq{intro_impl: HFormula #$\to$# (list literal * HFormula)}
...
\icoq{CNF-} \emph{i.e.}, a set of clauses modelling the elimination rule; and for a
negative literal (or the conclusion) \icoq{CNF+} \emph{i.e.}, a set of clauses
...
Except for \icoq{IMPL+}, the clausal encoding is also complete \emph{e.g.}, we have
\[
\begin{array}{lcl}
...
exception is when we intend to prove \icoq{False}.  In that case, the
\icoq{IMPL+} rule may drop the requirement on the decidability of the
...
$
is an intuitionistic tautology.% for ($f = (f_1 \to \dots \to f_n \to r)$).
...
%%%
%%Head/tail lists are an imperative data-structure; access to the first and last element of the list is in $O(1)$.
%%%
%%Our purely functional alternative is to have two distinguished
%%literals for the watched literals and store the rest of the literals
%%in a list.
%
...
result, each clause is annotated  with a set of literals \icoq{LitSet.t}.
%that is also represented using Patricia Trees.
\begin{minted}{coq}
...
$\Lit{f}$ for a \icoq{watched_map} $m$ such that $m[f] = (n,p)$.
...
literals \icoq{s} and that we have a clause $c$ of the form:
\[
...
$[y_1; y_n;\dots; y_n]$ a watched literal $y_i$ (\emph{i.e.}, $y_i$ is not assigned in \icoq{s})
...
\end{minted}
If the list is empty, we produce the unit clause \icoq{UNIT} $x$ and unit
...
%
Details about the components of \icoq{state} are given in Section~\ref{sec:solver-state}.
%
The \icoq{case_split} algorithm is parametrised by a prover \icoq{Prover:ProverT}.
It takes as input a clause \icoq{cl} and returns a prover performing a case-analysis over all the
...
state (\icoq{st} $\vdash$ d), the whole case-split is spurious and the
prover returns immediately.  Otherwise, we recursively call
\icoq{case_split} and return the updated set of literals needed by the proof.
%
...
literals.  A literal \icoq{f} in the list is of the form
...
aside during the  CNF conversion. % is incomplete if all the \icoq{a}s are not \emph{classical} propositions.
%
The prover is recursively called with, as goal,
the formula $\bigwedge_i a_i \to b$. If the proof succeeds, the literal \icoq{f} holds
...
\end{minted}
A \icoq{thy_prover} takes as input a hash-cons map \icoq{hm} and a
...
\item The field \icoq{arrows} contains a list of literals.  Each
...
\begin{example}[Example~\ref{exa:cnf} continued]
  ...
  \icoq{prover_arrows} with the singleton list $[\Lit{a \to c}]$.  To
...
%
The only axioms of the development are those of the \icoq{Int63} standard library 
...
We write $c_d$ (resp. $l_d$) for a (watched) clause
...
We also have to prove that the clauses \icoq{lc} obtained by theory reasoning are
sound. 
%
...
get a tactic that is similar to the existing \icoq{tauto} with the
...
%%   Consider the following goal.
%% \begin{minted}{coq}
%% Goal forall (x y:Z) (P: Prop), ((x = y) #$\to$# P) <#$\to$# (~ (x = y) \/ P).
...
the $q_j$ is an equality over \icoq{Z}, say \icoq{x=y}, we 
directly return to the SAT solver the tautology clause
\icoq{x<y #$\lor$# x=y #$\lor$# y<x}.
...
%% \begin{minted}{coq}
%% Inductive BForm : kind #$\to$# Type :=
%% | BTT : forall (k: kind), BForm k
%% | BFF : forall (k: kind), BForm k
%% | BAT : forall (k: kind), int #$\to$# BForm k
%% | BOP : forall (k: kind), op #$\to$# HCons.t (BForm k) #$\to$# HCons.t (BForm k) #$\to$# (BForm k)
%% | BIT : (BForm IsBool) #$\to$# BForm IsProp.
%% \end{minted}
...
%% \icoq{Z.ltb x y = true #$\to$# Z.lt x y}
%% becomes the propositional tautology \icoq{Z.lt x y #$\to$# Z.lt x y}.
...
proposition and therefore performs a case-split over \icoq{x>=0#$\to$# p}.
The first sub-case is solved by theory reasoning (\icoq{x=0 #$\to$# ~ x>= 0 #$\to$# False}
holds); and the second by propositional reasoning (\icoq{p #$\to$# p}).
%
...
%
As \icoq{a=b #$\to$# b=c #$\to$# a=c} can be proved by \icoq{congruence},
...
\begin{example} The following goal is solved by \icoq{intuition congruence}.
  ...
  expression \icoq{Z #$\to$# (False #$\leftrightarrow$# False)} is reified as an opaque
...
implicitly calls \icoq{reflexivity} and that our handling of theory reasoning
...
\icoq{itauto} outperforms the historic tactics for 40\% of the
cases. Overall, \icoq{itauto} is faster and there is an slight speedup of 1.07.
Yet, all the calls are quickly solved; the slowest goal is solved in 0.18s.

CompCert performs 924 calls to \icoq{itauto}.  For 76\% of the
...
%
For instance, the maximum running time of \icoq{tauto} is 5.26s to be
compared to 0.81s for \icoq{itauto}. Overall, \icoq{itauto} performs
better with a speedup of 2.8 for solving all the goals.

In summary, the results are rather positive though the advantage may be slim. %mixed and it is hard to draw definitive conclusions.
%
...
%
\icoq{itauto} spends time in different proof tasks: SAT solver, theory
